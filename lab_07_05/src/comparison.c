...
void collect_words(bst_node_t *root, char ***words, int *idx, int numbers_count) {
    if (!root || *idx >= numbers_count) return;
    
    collect_words(root->left, words, idx, numbers_count);
    
    (*words)[*idx] = str_dynamic_copy(root->word);
    (*idx)++;
    
    collect_words(root->right, words, idx, numbers_count);
}
...
comparison_results_t compare_structures(bst_node_t *bst_root, avl_node_t *avl_root, hst_chaining_t *hash_chaining, hst_open_t *hash_open) 
{
    ...
    if (!test_words) 
    {
        printf("Ошибка выделения памяти для test_words!\n");
        return results;
    }
    
    int index = 0;
    collect_words(bst_root, &test_words, &index, numbers_count);
...
void print_comparison_results(comparison_results_t *results) 
{
    ...
    printf("║ Время поиска (мкс)           ║ %8.2f ║ %8.2f ║ %8.2f ║ %8.2f ║\n", 
           results->bst_search_time, results->avl_search_time, 
           results->hash_chaining_search_time, results->hash_open_search_time);

    printf("║ Ср. сравнений на поиск       ║ %8d ║ %8d ║ %8d ║ %8d ║\n", 
           results->bst_comparisons, results->avl_comparisons, 
           results->hash_chaining_comparisons, results->hash_open_comparisons);
    ...
    printf("║ Высота/Загруженность         ║ %8d ║ %8d ║ %7.1f%% ║ %7.1f%% ║\n", 
           results->bst_height, results->avl_height, 
           results->hash_chaining_load_factor * 100, 
           results->hash_open_load_factor * 100);
...
void benchmark_structures(const char *filename, int hash_table_size) 
{
    ...
    free_bst(bst_root);
    free_avl(avl_root);
    if (hash_chaining) free_hash_table_chaining(hash_chaining);
    if (hash_open) free_hash_table_open(hash_open);
}
